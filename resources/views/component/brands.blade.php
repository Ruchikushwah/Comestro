<section class="w-full px-4 sm:px-10 md:px-20 py-10 bg-gray-100 font-sans flex justify-center items-center flex-col gap-8">
    <h1 class="text-3xl md:text-4xl font-bold text-gray-900 mb-4 bg-gradient-to-r from-[#0071bc] to-gray-600 bg-clip-text text-transparent">
        Brands We've Worked With
    </h1>

    <div class="flex flex-col md:flex-row bg-[#f0f4ff] rounded-xl overflow-hidden max-w-4xl w-full border-r-2 border-teal-200">

        <!-- Image Section -->
        <div class="flex justify-center items-center  md:w-1/2 p-4">
            <img src="ptpif.png" alt="Website Mockup" class="w-full h-auto rounded-lg shadow">
        </div>

        <!-- Description Section -->
        <div class="w-full md:w-1/2 p-6 md:p-10 flex flex-col justify-center">
            <h2 class="text-xl md:text-2xl font-semibold text-gray-800 mb-3">TechGotHire – Connecting Educators with Opportunities</h2>
            <p class="text-sm text-gray-600 leading-relaxed mb-4">
                TechGotHire is a platform designed to bridge the gap between skilled educators and institutions in need of top talent.
                With a streamlined hiring process, intuitive design, and AI-powered matching, we ensure that the right educators find
                the right opportunities efficiently and effectively.
            </p>
            <a href="https://ptpiui-gbdvdbbpe0hwh7gv.centralindia-01.azurewebsites.net/" target="_blank" class="text-[#ff6b6b] font-semibold hover:underline">Visit Website</a>
        </div>
    </div>

    <div class="flex flex-col md:flex-row bg-[#f0f4ff] rounded-xl overflow-hidden max-w-4xl w-full border-r-2 border-[#000000c9] ">

        <!-- Image Section -->
        <div class="flex justify-center items-center  md:w-1/2 p-4">
            <img src="\ssf.png" alt="Website Mockup" class="w-full h-auto rounded-lg shadow">
        </div>

        <!-- Description Section -->
        <div class="w-full md:w-1/2 p-6 md:p-10 flex flex-col justify-center">
            <h2 class="text-xl md:text-2xl font-semibold text-gray-800 mb-3">SmartStudio Photography – Capturing Moments, Creating Memories</h2>
            <p class="text-sm text-gray-600 leading-relaxed mb-4">
                SmartStudio Photography is dedicated to transforming special moments into timeless memories.
                With a keen eye for detail, artistic vision, and top-tier equipment, we bring out the best in every shot.
                Whether it's portraits, events, or commercial photography, our expertise ensures stunning, high-quality visuals.
            </p>
            <a href="https://smartstudiocreation.com" target="_blank" class="text-[#ff6b6b] font-semibold hover:underline">Visit Website →</a>
        </div>
    </div>

    <div class="flex flex-col md:flex-row bg-[#f0f4ff] rounded-xl overflow-hidden max-w-4xl w-full border-r-2 border-sky-200 ">

        <!-- Image Section -->
        <div class="flex justify-center items-center  md:w-1/2 p-4">
            <img src="\bsf.png" alt="Website Mockup" class="w-full h-auto rounded-lg shadow">
        </div>
        <!-- Description Section -->
        <div class="w-full md:w-1/2 p-6 md:p-10 flex flex-col justify-center">
            <h2 class="text-xl md:text-2xl font-semibold text-gray-800 mb-3">BiharShop – Your Ultimate Shopping Destination</h2>
            <p class="text-sm text-gray-600 leading-relaxed mb-4">
                BiharShop is an online shopping platform offering a wide range of products at unbeatable prices.
                With a seamless shopping experience, secure payments, and fast delivery, we bring convenience and quality right to your doorstep.
            </p>
            <a href="https://biharshop.com" class="text-[#ff6b6b] font-semibold hover:underline">Visit Website →</a>
        </div>
    </div><a href="#" 
   class="text-blue-600 font-semibold hover:underline px-3 py-1 rounded-md transition duration-200 ease-in-out hover:bg-gray-200 hover:text-gray-700 focus:ring-2 focus:ring-blue-600 focus:outline-none">
   See more →
</a>

</section>