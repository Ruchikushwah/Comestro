<div class="p-6 bg-gray-100 rounded-lg shadow-md w-full">
    ...
    <p class="text-sm"><strong>Status:</strong> <span
            class="{{ $ticket->status === 'closed' ? 'text-red-500' : 'text-green-500' }}">{{ $ticket->status }}</span>
    </p>
    ...
    @if ($ticket->status !== 'closed')
        ...
        <form wire:submit.prevent="sendMessage" class="mt-3">
            <textarea wire:model="newMessage" rows="3" placeholder="Type your message..."
                class="form-control w-full p-2 border rounded"></textarea>
